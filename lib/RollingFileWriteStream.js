...
class RollingFileWriteStream extends Writable {
  ...
  _writev(chunks, callback) {
    try {
      ...
      this.currentFileStream._writev(chunksForOldFile, e => error = e);
      if (error) {
      ...
      if (chunksForNewFile.length > 0) {
        ...
        this.currentFileStream._writev(chunksForNewFile, callback);
      } else {
        callback();
      }
    } catch (e) {
  ...
  _formatFileName(options) {
    const date = options.date;
    const index = options.index;
    const isHotFile = options.isHotFile;
    ...
    if (isHotFile) {
      if (this.options.alwaysIncludePattern && this._dateRollingEnabled()) {
        return this.options.keepFileExt
          ? [_.join([this.fileObject.name, dateStr], FILENAME_SEP), this.fileObject.ext].join('')
    ...
    if (this.options.keepFileExt) {
      ...
      fileName = [baseFileName, this.fileObject.ext].join('');
    } else {
  ...
  _roll(options) {
    const isNextPeriod = options.isNextPeriod;
    ...
    if (isNextPeriod) {
      ...
      targetFilePath = path.format({
        dir: this.fileObject.dir,
        base: this._formatFileName({date: this.state.currentDate, index: 0})
      });
