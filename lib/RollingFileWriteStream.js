...
function moveAndMaybeCompressFile(sourceFilePath, targetFilePath, needCompress, done) {
  ...
  fs.exists(sourceFilePath, (exists) => {
    ...
    debug(`moving file from ${sourceFilePath} to ${targetFilePath} ${needCompress ? 'with' : 'without'} compress`);
    if (needCompress) {
...
class RollingFileWriteStream extends Writable {
  ...
  _shouldRoll(bytesAdded, callback) {
    this.state.currentSize += bytesAdded;
    debug(`in _shouldRoll, pattern = ${this.options.pattern}, currentDate = ${this.state.currentDate}`);
  ...
  _final(callback) {
    debug(`_final called, file=${this.currentFileStream}`);
  ...
  _moveOldFiles(isNextPeriod, cb) {
    ...
    const done = () => {
      ...
      if (totalFilesToMove <= 0) {
        ...
        this.currentFileStream.write('', 'utf8', () => this._clean(cb)); // to touch file
      }
    };
    for (let i = totalFilesToMove; i >= 0; i--) {
      ...
      moveAndMaybeCompressFile(sourceFilePath, targetFilePath, this.options.compress && i === 0, done);
    }
  }
  ...
  _renewWriteStream(fileName, exists = false) {
    ...
    this.currentFileStream.on('drain', () => { this.emit('drain'); });
  }

  _clean(givenExistingFileDetails, cb = () => {}) {
    ...
    if (!Array.isArray(existingFileDetails)) {
      debug(`no passed in existing files`);
      cb = givenExistingFileDetails;
  ...
  _deleteFiles(fileNames, done) {
    if (fileNames) {
      let numToDelete = fileNames.length;
      fileNames.forEach(n => {
