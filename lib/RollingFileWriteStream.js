...
class RollingFileWriteStream extends Writable {
  ...
  _initState() {
    ...
    this._getExistingFiles((e, existingFileDetails) => {
      ...
      this.currentFileStream.write('', 'utf8', () => this._clean()); // to ensure file
    });
  }
  ...
  _moveOldFiles(isNextPeriod, cb) {
    ...
    const done = () => {
      totalFilesToMove -= 1;
      debug(`totalFilesToMove = ${totalFilesToMove}`);
  ...
  _clean(cb = () => {}) {
    this._getExistingFiles((e, existingFileDetails) => {
      if (e || existingFileDetails.length === 0) {
        cb();
        return;
      }
  ...
  _deleteFiles(fileNames, done) {
    if (fileNames) {
      ...
      fileNames.forEach(n => {
          ...
          fs.unlink(filePath, () => {
            numToDelete -= 1;
            debug(`${numToDelete} files remaining`);
            if (numToDelete === 0) {
              debug(`no more files to delete`);
              done();
            }
          });
      });
