...
class RollingFileWriteStream extends Writable {
  ...
  _write(chunk, encoding, callback) {
    try {
      if (this._dateRollingEnabled() && this.state.currentDate.clone()
        .endOf('day')
  ...
  _initState() {
    ...
    if (existingFileDetails.length === 0) {
      ...
      debug(`create new file with no file exists. name=${newFileName}`);
      this._renewWriteStream(newFileName);
    } else {
      const latestFileDetail = existingFileDetails[0];
      if (latestFileDetail.fileName === newFileName) {
        const hotFileState = fs.statSync(path.format({
          dir: this.fileObject.dir,
          name: newFileName
        }));
        const hotFileDate = moment(hotFileState.birthtimeMs);
        if (this._dateRollingEnabled() && !hotFileDate.isSame(now, 'day')) {
          if(hotFileDate.isBefore(now, 'day')) {
            this._moveFile(
              path.format(this.fileObject),
              path.format({
                dir: this.fileObject.dir,
                name: this._formatFileName({date: hotFileDate, index: 0})
              })
            );
          } else {
          ...
          debug(`create new file with hot file is out of date. name=${newFileName}`);
          this._renewWriteStream(newFileName);
        }
        ...
        debug(`using the existing hot file. name=${latestFileDetail.fileName}`);
        this._renewWriteStream(newFileName);
  ...
  _clean(givenExistingFileDetails) {
    ...
    if (this._dateRollingEnabled() && this.options.daysToKeep) {
      const outOfDateFileDetails = _.filter(
        existingFileDetails,
        f => f.date.clone().endOf('day').add(this.options.daysToKeep - 1, 'days').isBefore(moment())
      );
